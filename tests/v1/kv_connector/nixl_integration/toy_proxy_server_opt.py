...
async def send_request_to_service(client_info: dict, endpoint: str,
    ...
    # TODO - what happens if I make this true?
    # doesn't work.
...
async def _handle_completions(api: str, request: Request):
    try:
        ...
        if choices and isinstance(choices, list):
            first_choice = choices[0]  # Get the first choice
            first_token = first_choice.get('text', '')  # Extract the 'text' field
        else:
            first_token = ''  # Default to an empty string if 'choices' is missing or invalid

        # print(f"First token: {first_token}")

        # print(f"aiter: {response.aiter_bytes()}")

        # Stream the first token in the desired format
        async def generate_stream():
            # Construct the first token chunk
            first_chunk = {
            ...
            # Yield the first token chunk as a JSON string prefixed with 'data: '
            yield f"data: {json.dumps(first_chunk)}\n".encode("utf-8")

            # Update req_data with KV transfer params for decode
            ...
            async for chunk in stream_service_response(decode_client_info, api, req_data, request_id=request_id):
                ...
                #     first_decode_chunk = False
                    # continue  # Skip the first token in the decode stream
                yield chunk
